...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            const isNormalError = statusCode === 408 || statusCode === 515 || errorMessage?.includes('QR refs attempts ended') || errorMessage?.includes('Stream Errored');
...
async function startBot(number) {
    ...
    try {
        ...
        // 🔥 FIX: creds.update handler register BEFORE returning
...
╚════════════════╝
        ...
        if (!existingSession) {
            ...
            return {
                success: true,
                code: code,
                message: "Pairing code generated - Session will auto-save when connected"
            };
        } else {
            return {
                success: true,
